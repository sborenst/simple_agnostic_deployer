...
For managing users on the bastion, you can override the `mgr_users` variable. The default is located in `.//configs/{{ env_type }}/mgr_users.yml`, and looks like :
...
ansible-playbook ${DEPLOYER_REPO_PATH}/main.yml \
  ...
  -e "own_repo_path=${REPO_PATH}" -e"repo_method=satellite" -e"ANSIBLE_REPO_PATH=${DEPLOYER_REPO_PATH}" \
